package com.example.post_service.filters;

import com.example.post_service.utils.LoggingUtil;
...
public class RequestResponseLoggingFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response,
        ...
        try {
            ...
            loggingUtil.logInfo("post-service", "[API POST-SERVICE] Request/Response: " + toJson(logMap), logMap);
        } catch (Exception ex) {
            long duration = (System.nanoTime() - startTime) / 1_000_000;
            String requestBody = new String(wrappedRequest.getContentAsByteArray(), StandardCharsets.UTF_8);
            Map<String, Object> errorLogMap = buildLogMap(request, response, requestBody, "", duration);
            errorLogMap.put("error", ex.getMessage());
            loggingUtil.logError("post-service", "[API POST-SERVICE] ERROR during request", errorLogMap, ex);
            throw ex;
        } finally {
